...
_ERROR_SEVERITIES: Dict[str, str] = {
    ...
    # ERRORES DE POLÍTICAS (CON RECLASIFICACIONES DE SEVERIDAD)
...
class ValidatePolicyStage(LLMStage):
    ...
    def _prepare_llm_input(self, item: Item) -> str:
        ...
        # La entrada para el LLM debe contener el ítem.
        llm_input_payload = {
            "item_id": str(item.temp_id), # Usar temp_id para el LLM
    ...
    async def _process_llm_result(self, item: Item, result: ValidationResultSchema):
        ...
        if processed_findings:
            ...
            if has_fatal_finding:
                ...
                # Si no hay findings fatales, pero sí otros errores/advertencias, el outcome es "fail" (porque is_valid=false)
